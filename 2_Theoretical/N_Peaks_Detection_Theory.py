...
for i, (u, sig, cL, cU) in enumerate(peak_params):
...
for index, AoN in enumerate(AoNs):
    ...
    for _ in range(Nsim_per_AoN):
        ...
        amplitudes = []
        for (u, sig, A_lower, A_upper), ratio, i in zip(peak_params, A_ratio, range(peaks_no)):
            if dependant[i] == i:
        ...
        for (u, sig, A_lower, A_upper), ratio, i in zip(peak_params, A_ratio, range(peaks_no)):
            if np.isnan(amplitudes[i]):
                ratio = np.random.uniform(ratio[0], ratio[1]) * amplitudes[dependant[i]]
                model += gaussian(x, A * ratio, u, sig, sig_resolution)
        ...
        # Loop again for amplitudes of dependant peaks - have to do afterwards as could be dependant on a line appearing after it in the file
        for i in range(peaks_no):
            if dependant[i] != i:
                ...
                if peak_params[i][2] == peak_params[i][3]:
                    ...
                    pfit.add(f'delta{i+1}', value = (peak_params[i][2] + peak_params[i][3]) / 2, min = peak_params[i][2], max = peak_params[i][3], vary = True)
...
for i in range(len(AoNs_out)):
    plt.title(f'{label} against A/N for line {i+1} for Nsim = {Nsim}')
    plt.scatter(AoNs_out[i], array[0], s=0.5)
    plt.xlabel('A/N')
    plt.ylabel('%s' % label)
    std = np.std(array)
    plt.xlim(-0.2, 11)
