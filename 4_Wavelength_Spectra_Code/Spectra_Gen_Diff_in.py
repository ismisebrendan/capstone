...
lam_min = min(p[0] * (1 + p[3]/c) for p in peak_params)
lam_max = max(p[0] * (1 + p[3]/c) for p in peak_params)
...
lams_in = np.array([p[0] for p in peak_params])
...
lams_unc_out = np.empty((peaks_no, Nsim))
AoNs_out = np.empty((peaks_no, Nsim))
AoNs_unc_out = np.empty((peaks_no, Nsim))
...
for index, AoN in enumerate(AoNs):
    ...
    for _ in range(Nsim_per_AoN):
        ...
        for peak in range(peaks_no):
            As_out[peak][index] = fit.params[f'g{peak+1}_A'].value
            As_unc_out[peak][index] = fit.params[f'g{peak+1}_A'].stderr
            sigs_out[peak][index] = fit.params[f'g{peak+1}_sig'].value
            sigs_unc_out[peak][index] = fit.params[f'g{peak+1}_sig'].stderr
            vels_out[peak][index] = fit.params[f'g{peak+1}_vel'].value
            vels_unc_out[peak][index] = fit.params[f'g{peak+1}_vel'].stderr
            lams_out[peak][index] = fit.params[f'g{peak+1}_lam_rf'].value
            lams_unc_out[peak][index] = fit.params[f'g{peak+1}_lam_rf'].stderr
...
for peak in range(peaks_no):
    ...
    AoNs_unc_out[peak] = As_unc_out[peak]/np.sqrt(bkg)
...
data = [As_in, As_out, As_unc_out, AoNs, AoNs_out, AoNs_unc_out, lams_in, lams_out, lams_unc_out, sigs_in, sigs_out, sigs_unc_out, vels_in, vels_out, vels_unc_out, peaks_no]
