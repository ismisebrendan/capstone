...
def gaussian(x, A, u, sig, sig_resolution):
    ...
    return np.abs(A) * np.exp(-0.5*(x - u)**2 / sig_obs**2)
...
for i in range(peaks_no):
    A_ratio.append([peak_params[i][2], peak_params[i][3]])
...
for i, (u, sig, AL, AU, cL, cU) in enumerate(peak_params):
...
for index, AoN in enumerate(AoNs):
    ...
    for _ in range(Nsim_per_AoN):
        ...
        amplitudes = []
        for (u, sig, AL, AU, cL, cU), ratio, i in zip(peak_params, A_ratio, range(peaks_no)):
            if dependant[i] == i:
                ratio = np.random.uniform(AL, AU)
        ...
        for (u, sig, AL, AU, cL, cU), ratio, i in zip(peak_params, A_ratio, range(peaks_no)):
            if np.isnan(amplitudes[i]):
                ratio = np.random.uniform(AL, AU) * amplitudes[dependant[i]]
                model += gaussian(x, A * ratio, u, sig, sig_resolution)
        ...
        for i in range(1, peaks_no):
            ...
            if dependant[i] == i:
                ...
                pfit.add(f'g{i+1}_sig', value=sig) #expr='g1_sig') 
        ...
        for i in range(peaks_no):
            if dependant[i] != i:
                ...
                if peak_params[i][2] == peak_params[i][3]:
                    pfit.add(f'g{i+1}_A', expr=f'g{dependant[i]+1}_A * {A_ratio[i][0]}')
                else:
                    pfit.add(f'delta{i+1}', value = (peak_params[i][4] + peak_params[i][5]) / 2, min = peak_params[i][4], max = peak_params[i][5], vary = True)
        ...
        # Find amplitudes of the different lines
        for u in range(peaks_no):
            # H alpha
            if peak_params[u][0] == 656:
                Halpha_amp = fit.params[f'g{u+1}_A'].value
            # H beta
            if peak_params[u][0] == 486:
                Hbeta_amp = fit.params[f'g{u+1}_A'].value
            # OIII
            if peak_params[u][0] == 501:
                OIII_amp = fit.params[f'g{u+1}_A'].value
            # NII
            if peak_params[u][0] == 658:
                NII_amp = fit.params[f'g{u+1}_A'].value
            
        Seyfert = np.log10(OIII_amp / Hbeta_amp) > 1.05 * np.log(NII_amp / Halpha_amp) + 0.45
        ...
        plt.title(f'Seyfert galaxy: {Seyfert}')
        plt.plot(x, y, 'k-')
        plt.plot(x, model, 'c-')
...
array = dsigs / sigs_in
i=(np.abs(AoNs_out[0]-5) <0.5)
print(np.std(array[0][i]))
label = '(sig_out - sig_in)/sig_in'
plt.title(f'{label} against A/N for for Nsim = {Nsim}')
