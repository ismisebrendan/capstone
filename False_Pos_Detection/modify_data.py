...
# Number of files from each folder to combine and modify, takes from start_file to end_file
start_file = 0
end_file = 9
...
if len(folders) == 1:
    ...
    np.savetxt(f'{fold}/{fold}_{start_file}_to_{end_file}_mod.txt', data_out, header=f'# {fold} region lines from files {start_file} to {end_file}')
elif len(folders) == 4:
    files_present = [f for f in os.listdir('.') if os.path.isfile(os.path.join('.', f)) and f[-4:] == ".txt" and 'spectra_all' in f and '_0' or '_1' in f]
    np.savetxt(f'spectra_{start_file}_to_{end_file}_mod.txt', data_out, header=f'# Lines from files {start_file} to {end_file} for all regions generated till {datetime.datetime.now()}')
else:
    ...
    np.savetxt(f'spectra_{start_file}_to_{end_file}_{"_".join(folders)}_{len(files_present)}_mod.txt', data_out, header=f'# Lines from files {start_file} to {end_file} from {" ".join(folders)}')
