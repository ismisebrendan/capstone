...
spec = Spectrum('../lines_in_PN.txt', '../fitting_PN.txt', Nsim=Nsim)
...
files_present = [f for f in os.listdir() if os.path.isfile(os.path.join(".", f)) and f[:16] == "planetary_nebula" and f[-4:] == ".txt"]
file_out = f'planetary_nebula{len(files_present)}.txt'

np.savetxt(file_out, data_out, header=f'first {Nsim} are full spectra, next {Nsim} have random lines missing, next {Nsim} is just noise')